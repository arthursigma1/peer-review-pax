...
- **Consulting source enforcement (PS-VD-9xx).** Sources with IDs in the PS-VD-900–999 range are consulting/industry-report sources from `consulting_context.json`. They carry `allowed_usage: [market_context, distribution_context, operating_model_context, supporting_context]` and `disallowed_usage: [firm_specific_fact, firm_specific_metric, firm_specific_action, sole_basis_for_recommendation]`. If a claim cites ONLY PS-VD-9xx sources AND the claim is about a specific firm's metric, action, or causal attribution, verdict is `UNGROUNDED`. If a claim cites PS-VD-9xx + peer sources, and the peer source supports the core fact, the claim may pass. Market-level claims citing only PS-VD-9xx sources are permitted. Each consulting claim also has a `scope` field (`market`, `segment`, `multi_firm`, `single_firm`); claims with `single_firm` scope used as firm-specific evidence receive `UNGROUNDED`.
