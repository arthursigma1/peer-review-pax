...
const DEFAULT_CONFIG: AgentConfig = {
  agents: [
    ...
    },
    {
      ...
      instructions: "Deep-dive the alt-manager verticals through strategy sub-types, not monolithic asset classes. Use docs/pax-peer-strategy-ontology.md as the minimum decomposition grid and docs/pax-peer-assessment-framework.md as the business framing for what the deep-dive should explain. For each vertical, segment by strategy_sub_type, thematic_focus, and economic_model, then map the combinations of business-model choices that appear associated with KPI outcomes. Do not collapse the analysis below the ontology baseline without explicit disclosure. Document value-creation mechanics, fee model, operating model, tech/data/reporting requirements, scaling constraints, margin sensitivities, and transferability barriers for PAX. Ground all claims in specific peer actions and keep credit, PE, infrastructure, real estate, and natural resources broken into the sub-types that actually matter for transferability. Treat secondaries, GP-leds, co-investments, and other solutions-style programs as strategy sub-types or cross-vertical wrappers inside the relevant asset class unless a run-specific override is disclosed.",
    ...
    },
    {
      ...
      instructions: "Sequential outputs: (1) Value Principles — for each headline driver, state the exact statistical status using the repository contract, disclose mechanical overlap and confounding where relevant, and add PAX relevance plus driver decomposition logic. Respect docs/pax-peer-assessment-framework.md as the business brief: keep the synthesis useful for both platform and BU readers, and preserve the distinction between reinforce current strategies, bring additional angles and opportunities, and suggest new initiatives. (2) Platform Playbook — produce PLAY-* and ANTI-* entries that include the full required contract: What_Was_Done, Observed_Metric_Impact, Why_It_Worked, PAX_Relevance, Preconditions, Operational_And_Tech_Prerequisites, Execution_Burden, Time_To_Build, Margin_Risk, Failure_Modes_And_Margin_Destroyers, Transferability_Constraints, Archetype_Relevance, Evidence_Strength, and Recommendation_For_PAX. (3) Asset Class Playbooks — same contract, but organized by vertical and strategy sub-type using docs/pax-peer-strategy-ontology.md as the minimum decomposition baseline. Do not call a driver stable unless stable_v1_two_of_three is satisfied. Every recommendation must explicitly state why it matters for PAX, what would have to be true, why it may fail, and whether it is near_term_feasible, medium_term_feasible, or aspirational.",
